...
##################################################################################
def extract_part_org_relation(data, inputFile):
    ...
    for sent in sents:
        ...
        for ent in inp_doc.ents:
            if(ent.label_ == "ORG" and e_list.get(ent.text) is None):
...
if __name__ == '__main__':
    ...
    part_data       = extract_part_org_relation(inp, inputFile)
    json_object     = json.loads(json.dumps(part_data))
    formatting_data = json.dumps(json_object, indent=4)
    file            = open('output/task2/PART_OF/PART_OF_ORG/outORG.txt', "w")
